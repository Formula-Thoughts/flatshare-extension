...
from backend.src.exceptions import AutoFixtureException
...
class ObjectMapper:
    def map_to_dict_and_ignore_none_fields(self, _from, to: typing.Type[T]) -> dict:
        ...
        mapped = self.__generic_map(_from=_from, to=to, propValues=vars(_from).items())
            ...
            try:
                if bool(typing.get_type_hints(getattr(mapped, property))):
                    self.__to_dict_and_ignore_none_fields(
                        new_dict=value, mapped=getattr(mapped, property)
                    )
            except TypeError:
    ...
    def map(self, _from, to: typing.Type[T]) -> T:
        print(f"{vars(_from).items()}")
        return self.__generic_map(_from=_from, to=to, propValues=vars(_from).items())

    def map_from_dict(self, _from, to: typing.Type[T]) -> T:
        print(_from.items())
        return self.__generic_map(_from=_from, to=to, propValues=_from.items())
            ...
            if property in dict_to:
                if bool(typing.get_type_hints(dict_to[property])):
                    setattr(
                        new_dto,
                        property,
                        map_callback(self.map(_from=value, to=dict_to[property])),
                    )
                    ...
                    for item in value:
                        collection.append(
                            map_callback(self.map(_from=item, to=sub_item_to))
                        )
                    setattr(new_dto, property, collection)
                else:
                    new_dto.__dict__[property] = value
        print(
            f"__generic_map from {type(_from)} {to} and mapped {_from} out -> {new_dto}"
        )
        return map_callback(new_dto)


class AutoFixture:
    def create_many_dict(
        self, dto, ammount, seed=None, num=None, nest=0, list_limit=100
    ):
        many = self.create_many(
            dto=dto,
            ammount=ammount,
            seed=seed,
            num=num,
    ...
    def create_dict(self, dto, seed=None, num=None, nest=0, list_limit=100):
        return self.create(
            dto=dto, seed=seed, num=num, nest=nest, list_limit=list_limit
        ).__dict__

    def create_many(self, dto, ammount, seed=None, num=None, nest=0, list_limit=100):
        list_of_dtos = []
    ...
    def create(
        self, dto: typing.Type[T], seed=None, num=None, nest=0, list_limit=10
    ) -> T:
        ...
        try:
            ...
            raise AutoFixtureException(
                "class must empty ctor, if a dataclass, must have fields initialised to "
        ...
        for key, _type in members:
            if (getattr(new_value, key) is None) or (
                ...
                if _type is str:
                    self.__generate_string_field(
                        is_predictable_data, key, new_value, seed
                    )
                ...
                if _type == datetime.datetime:
                    self.__generate_datetime_field(
                        is_predictable_data, key, new_value, num
                    )
                ...
                if _type is float:
                    self.__generate_float_field(
                        is_predictable_data, key, new_value, num
                    )

                if _type == list[str]:
                    self.__generate_str_list_field(
                        is_predictable_data, key, new_value, num, seed, list_limit
                    )

                if _type == list[int]:
                    self.__generate_int_list_field(
                        is_predictable_data, key, new_value, num, list_limit
                    )

                if _type == list[bool]:
                    self.__generate_bool_list_field(
                        is_predictable_data, key, new_value, num, list_limit
                    )

                if _type == list[datetime.datetime]:
                    self.__generate_datetime_list_field(
                        is_predictable_data, key, new_value, num, list_limit
                    )

                if _type == list[float]:
                    self.__generate_float_list_field(
                        is_predictable_data, key, new_value, num, list_limit
                    )

                if type(_type) is type(Enum):
                    self.__generate_random_enum_field(
                        _type, is_predictable_data, key, new_value, num
                    )
                ...
                if typing.get_origin(_type) is list:
                    ...
                    if type(arg) is type(Enum):
                        self.__generate_list_of_enums_field(
                            arg, is_predictable_data, key, new_value, num, list_limit
                        )
                    if bool(typing.get_type_hints(arg)):
                        self.__generate_class_list(
                            arg,
                            is_predictable_data,
                            key,
                            nest,
    ...
    def __generate_class_field(self, _type, key, nest, new_value, num, seed):
        setattr(
            new_value, key, self.create(dto=_type, seed=seed, num=num, nest=nest + 1)
        )

    def __generate_class_list(
        self, _type, is_predictable_data, key, nest, new_value, num, seed, list_limit
    ):
        if is_predictable_data:
            value_for_given_member = []
            for i in range(0, num):
                value_for_given_member.append(
                    self.create(dto=_type, seed=seed, num=num, nest=nest + 1)
                )
        else:
            value_for_given_member = []
    ...
    def __generate_list_of_enums_field(
        self, _type, is_predictable_data, key, new_value, num, list_limit
    ):
        if is_predictable_data:
            value_for_given_member = []
            for i in range(0, num):
    ...
    def __generate_random_enum_field(
        self, _type, is_predictable_data, key, new_value, num
    ):
        if is_predictable_data:
            enum_iterable = list(_type)
            length = len(enum_iterable)
    ...
    def __generate_datetime_list_field(
        self, is_predictable_data, key, new_value, num, list_limit
    ):
        if is_predictable_data:
            value_for_given_member = []
            for i in range(0, num):
    ...
    def __generate_bool_list_field(
        self, is_predictable_data, key, new_value, num, list_limit
    ):
        if is_predictable_data:
            value_for_given_member = []
    ...
    @staticmethod
    def __generate_float_list_field(
        is_predictable_data, key, new_value, num, list_limit
    ):
        if is_predictable_data:
            value_for_given_member = []
    ...
    def __generate_str_list_field(
        self, is_predictable_data, key, new_value, num, seed, list_limit
    ):
        if is_predictable_data:
            value_for_given_member = []
            for i in range(0, num):
            ...
            for i in range(0, random.randint(0, list_limit)):
                value_for_given_member_item = key
                value_for_given_member_item = (
                    f"{value_for_given_member_item}{self.__generate_random_seed()}"
                )
                value_for_given_member.append(value_for_given_member_item)
        setattr(new_value, key, value_for_given_member)
    ...
    def __validate_predictable_data(num, seed):
        if seed is not None and num is None:
            raise AutoFixtureException(
                "seed and num must be both set to create predictable data"
            )
        if seed is not None and num is None:
            raise AutoFixtureException(
                "seed and num must be both set to create predictable data"
    ...
    def __generate_string_field(self, is_predictable_data, key, new_value, seed):
        ...
        if is_predictable_data:
            ...
            value_for_given_member = (
                f"{value_for_given_member}{self.__generate_random_seed()}"
            )
        setattr(new_value, key, value_for_given_member)
...
class JsonSnakeToCamelSerializer:
    ...
    def __snake_case_to_camel_case_dict(self, d):
        if isinstance(d, list):
            return [
                self.__snake_case_to_camel_case_dict(i)
                if isinstance(i, (dict, list))
                else self.__format_value(i)
                for i in d
            ]
        return {
            self.__snake_case_key_to_camel_case(
                a
            ): self.__snake_case_to_camel_case_dict(b)
            if isinstance(b, (dict, list))
            else self.__format_value(b)
            for a, b in d.items()
        }
...
class JsonCamelToSnakeCaseDeserializer:
    ...
    def __camel_case_to_snake_case_dict(self, d):
        if isinstance(d, list):
            return [
                self.__camel_case_to_snake_case_dict(i)
                if isinstance(i, (dict, list))
                else i
                for i in d
            ]
        return {
            self.__camel_case_key_to_snake_case(
                a
            ): self.__camel_case_to_snake_case_dict(b)
            if isinstance(b, (dict, list))
            else b
            for a, b in d.items()
        }
